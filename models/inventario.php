...
// convierte a minúsculas toda una cadena la función strtolower
    ...
    function insertar() {
        ...
        try {
            ...
            $orden_venta_detalle_ID='NULL';
            if($this->orden_venta_detalle_ID!=null){
                $orden_venta_detalle_ID=$this->orden_venta_detalle_ID;
            }
            $compra_detalle_ID='NULL';
            if($this->compra_detalle_ID!=null){
                $compra_detalle_ID=$this->compra_detalle_ID;
            }
            ...
            $q = 'insert into inventario(ID,empresa_ID, compra_detalle_ID,orden_venta_detalle_ID, descripcion, producto_ID,estado_ID,';
            $q.= 'utilidad_soles,utilidad_dolares,comision_soles,comision_dolares,serie,cotizacion_detalle_ID,usuario_id )';
            $q.='values('.$ID.','.$_SESSION['empresa_ID'].','. $compra_detalle_ID .','.$orden_venta_detalle_ID. ',"'. $this->descripcion. '",' .$this->producto_ID.
   ...
   function actualizar(){
        ...
        try{
                ...
                $q="UPDATE inventario SET producto_ID=".$this->producto_ID.",compra_detalle_ID=".$compra_detalle_ID;
                $q.=",orden_venta_detalle_ID=".$orden_venta_detalle_ID;
    ...
    static function getByID($ID)
    {
		...
		try 
		{
			$q='select ID, producto_ID,compra_detalle_ID ,orden_venta_detalle_ID,';
			...
			foreach($dt as $item)
			{
				...
				$oInventario->producto_ID=$item['producto_ID'];
                                $oInventario->compra_detalle_ID=$item['compra_detalle_ID'];
				$oInventario->orden_venta_detalle_ID=$item['orden_venta_detalle_ID'];
    ...
    static function getByID2($compra_detalle_ID)
    {
           $cn =new connect();
           try 
           {
                   $q='  select ID, producto_ID,compra_detalle_ID, orden_venta_detalle_ID,cantidad_ingreso,'
                           . '  cantidad_salida, stock, fecha,descripcion,cotizacion_detalle_ID,ifnull(usuario_mod_id,-1) as usuario_mod_id ';
                   $q.=' from inventario where del=0 and compra_detalle_ID='.$compra_detalle_ID;
    ...
    static function getGridInventario($filtro = '', $desde = -1, $hasta = -1, $order = 'inv.ID asc') {
        ...
        try {
            ...
            $q.="(case when inv.compra_detalle_ID is null then '' else cd.ID end ) as compra_detalle,";
            $q.="(case when inv.compra_detalle_ID is null then '' else co.numero end ) as numero_compra,";
            $q.="(case when inv.orden_venta_detalle_ID is null then '' else ovd.ID end ) as orden_venta_detalle_ID,";
            $q.="(case when inv.orden_venta_detalle_ID is null then '' else ov.numero_concatenado end ) as numero_orden_venta,";
            $q.="ifNull((case when inv.orden_venta_detalle_ID is null then '' else fv.numero_concatenado end ),'') as numero_factura_venta,";
            $q.="ifNull((case when inv.orden_venta_detalle_ID is null then '' else gv.numero_concatenado end ),'') as numero_guia_venta,";
            ...
            $q.=" from inventario inv left join compra_detalle cd on inv.compra_detalle_ID=cd.ID";
            $q.=" left join compra co on cd.compra_ID=co.ID";
            $q.=" left join orden_venta_detalle ovd on inv.orden_venta_detalle_ID=ovd.ID ";
            $q.=" left join orden_venta ov on ovd.orden_venta_ID=ov.ID";
            $q.=" left join factura_venta fv on fv.orden_venta_ID=ov.ID";
            $q.=" left join guia_venta gv on ov.ID=gv.orden_venta_ID";
    ...
    static function getGridInventario1($filtro = '', $desde = -1, $hasta = -1, $order = 'inv.ID asc') {
        $cn = new connect();
        try {
            $q="select inv.ID,inv.producto_ID,ifnull(inv.compra_detalle_ID,-1) as compra_detalle_ID ,ifnull(inv.orden_venta_detalle_ID,-1) as orden_venta_detalle_ID,";
     ...
     static function getGridProductos($filtro = '', $desde = -1, $hasta = -1, $order = 'pro.nombre asc') {
        ...
        $cn = new connect();
        try {
            $q='select inv.ID, inv.producto_ID,inv.compra_detalle_ID,inv.orden_venta_detalle_ID,cod.descripcion,cod.ID as codigo_compradetalle, co.ID as codigo_compra,inv.estado_ID ';
            $q.=' from inventario inv, compra_detalle cod, compra co ';
            $q.=' where inv.del=0 and co.ID = cod.compra_ID and cod.ID = inv.compra_detalle_ID ';
            if ($filtro != '') {
                $q.=' and ' . $filtro;
            }
      ...
      static function getGridComprobarVenta($filtro = '', $desde = -1, $hasta = -1, $order = 'inv.ID asc') {
        ...
        try {
            $q='select inv.ID, inv.producto_ID,inv.compra_detalle_ID,inv.orden_venta_detalle_ID,cod.descripcion,cod.ID as codigo_compradetalle, co.ID as codigo_compra,inv.estado_ID ';
            $q.=' from inventario inv, compra_detalle cod, compra co ';
            $q.=' where inv.del=0 and co.ID = cod.compra_ID and cod.ID = inv.compra_detalle_ID and inv.orden_venta_detalle_ID !="" and inv.compra_detalle_ID !="" ';
            if ($filtro != '') {
                $q.=' and ' . $filtro;
            }
    ...
    function actualizar2($compra_detalle_ID){
        $cn =new connect();
        $retornar=-1;
        try{
            $fecha='NULL';
                ...
                $q.=" usuario_mod_id=".$this->usuario_mod_id.", fdm=Now()";
                $q.=" WHERE compra_detalle_ID=".$compra_detalle_ID;
               // echo $q;
                $retornar=$cn->transa($q);
                $this->getMessage='Se guardó correctamente';
                $q="select ID from inventario where del=0 and compra_detalle_ID=".$compra_detalle_ID;
                $ID=$cn->getData($q);        
                $this->ID=$ID;
                return $retornar;
        }
    ...
    function eliminar2(){
         $cn =new connect();
            ...
            try{
                ...
                $q='UPDATE inventario SET cantidad_ingreso='.$this->cantidad_ingreso.',stock='.$this->stock.',compra_detalle_ID=null, usuario_mod_id='.$this->usuario_mod_id.', fdm=Now()';
                $q.=' WHERE ID='.$this->ID;
    ...
    function getStock($producto_ID){
        $cn =new connect();
         try 
         {
                ...
                $q="select count(inv.ID) from inventario inv INNER JOIN compra_detalle cd ON inv.compra_detalle_ID=cd.ID where inv.del=0 and cd.del=0 and inv.producto_ID=".$producto_ID." and inv.estado_ID=48" ;
                $existencia=$cn->getData($q);
                $vendidos=$cn->getData("select count(inv.ID) from inventario inv  INNER JOIN orden_venta_detalle ovd ON inv.orden_venta_detalle_ID=ovd.ID where inv.del=0 and ovd.del=0 and inv.producto_ID=".$producto_ID." and inv.estado_ID=50");
    ...
    function getGuiaFaltanteSeries($producto_ID){
        $cn =new connect();
         try 
         {
                 $q='select inv.ID, inv.producto_ID,inv.orden_venta_detalle_ID,inv.cantidad_salida,inv.stock,DATE_FORMAT(inv.fecha,"%d-%m-%Y") as fecha,inv.descripcion,';
                 $q.='ov.ID as orden_venta_ID,ov.numero ';
                 $q.='from inventario inv,orden_venta ov,orden_venta_detalle ovd ';
                 $q.='where inv.orden_venta_detalle_ID=ovd.ID and ovd.orden_venta_ID=ov.ID and inv.del=0 and ov.del=0 and ovd.del=0  and inv.producto_ID='.$producto_ID.' and inv.tipo="salida" ';
    ...
    function getOrden_Venta_Detalle($producto_ID,$compra_detalle_ID){
         $cn =new connect();
         try 
         {
                $q='select count(inv.ID) as cantidad,group_concat(inv.ID separator"|") as IDs,inv.orden_venta_detalle_ID from inventario inv';
                $q.=' LEFT JOIN orden_venta_detalle ovd ON inv.orden_venta_detalle_ID=ovd.ID';
                $q.=' where inv.del=0  and ';
                if($compra_detalle_ID==0){
                    $q.='inv.estado_ID=50';
                }else {
                     $q.='(inv.estado_ID=50 or inv.compra_detalle_ID='.$compra_detalle_ID.')';
                }
                $q.=' and inv.producto_ID='.$producto_ID.' and ovd.del=0';
                $q.=' group by orden_venta_detalle_ID ';
    ...
    function getReporteComisiones(){
         ...
         try 
         {
                ...
                $q.=' from inventario inv , orden_venta_detalle ovd,orden_venta ov, compra_detalle cod,compra co ';
                $q.=' where inv.del=0 and ov.ID=ovd.orden_venta_ID and ovd.ID=inv.orden_venta_detalle_ID and inv.compra_detalle_ID=cod.ID and cod.compra_ID=co.ID  ';
                $q.=' and inv.estado_ID=49 order by inv.orden_venta_detalle_ID asc ';
    ...
    static function getMovimientoProducto($producto_ID) {
        ...
        try {
            ...
            $q.=" where cd.compra_ID=co.ID and cd.producto_ID=pro.ID and co.moneda_ID=mo.ID and cd.del=0 and cd.producto_ID=".$producto_ID.");";
            //echo $q;
            /*$q.=" UNION ";*/
            $cn->transa($q);
            ...
            $q.=" from producto pro,orden_venta ov,  orden_venta_detalle ovd, moneda mo,factura_venta fv";
            $q.=" where ovd.orden_venta_ID=ov.ID and ovd.producto_ID=pro.ID and fv.orden_venta_ID=ov.ID and ov.moneda_ID=mo.ID and ovd.del=0 and ovd.producto_ID=".$producto_ID.");";
            $cn->transa($q);
            //echo $q;
