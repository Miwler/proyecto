...
//7  Obsequio (se convierte en obsequio el costo de venta es 0)
    ...
    Private $message;
    private $oMoneda;
    private $orden_venta_detalle_padre_ID;
    private $oProducto;
    private $oOrden_Venta;
...
// convierte a minúsculas toda una cadena la función strtolower
        ...
        if (property_exists('orden_venta_detalle', $temporal)) {
            $this->$temporal = $valor;
        } else {
            echo $var . " No existe.";
        }
    ...
    function insertar() {
        ...
        $retorna = -1;
        try {
            $q = 'select ifnull(max(ID),0)+1 as ID from orden_venta_detalle;';
            $ID=$cn->getData($q);
            
            $q = 'insert into orden_venta_detalle (ID,producto_ID,observacion,orden_venta_ID,descripcion,cantidad,';
            $q.='precio_venta_unitario_soles,precio_venta_unitario_dolares,precio_venta_subtotal_soles,precio_venta_subtotal_dolares,';
            $q.='precio_venta_soles,precio_venta_dolares,igv,vigv_soles,vigv_dolares,orden_venta_detalle_ID,ver_precio,estado_ID,cotizacion_detalle_ID,';
            $q.='usuario_id,tipo)values ';
            $q.='('.$ID.','.$this->producto_ID.',"'.$this->observacion.'",'.$this->orden_venta_ID.',"'.FormatTextSave($this->descripcion).'",'.$this->cantidad.',';
            ...
            $q.=$this->precio_venta_soles.','.$this->precio_venta_dolares.','.$this->igv.','.$this->vigv_soles.','.$this->vigv_dolares.','.$this->orden_venta_detalle_ID.',';
    ...
    function actualizar() {
        ...
        try {
            ...
            $q = 'update orden_venta_detalle set producto_ID='.$this->producto_ID.',descripcion="'.FormatTextSave($this->descripcion);
            ...
            $q.=',orden_venta_detalle_ID='.$this->orden_venta_detalle_ID.',estado_ID='.$this->estado_ID.', cotizacion_detalle_ID='.$cotizacion_detalle_ID .',ver_precio='.$this->ver_precio.',tipo='.$this->tipo.',usuario_mod_id='.$this->usuario_mod_id;
	...
	$retornar=-1;
        try{
            $q='update orden_venta_detalle set precio_venta_unitario='.$this->precio_venta_unitario.', precio_venta='.$this->precio_venta.', usuario_mod_id='. $this->usuario_mod_id;
            $q.=', fdm=now() where del=0 and id='.$this->ID;
            $retornar=$cn->transa($q);
            $this->message='Se guardó correctamente';
            //echo $q;
	...
	$retornar=-1;
        try{
            $q='update orden_Venta_detalle set tipo='.$this->tipo;
            $q.=' where ID='.$this->ID;
            $retornar=$cn->transa($q);
            //$this->message='Se guardó correctamente';
            //echo $q;
    ...
    function actualizarCK() {
        ...
        $retornar = -1;
        try {
            $q = 'update orden_venta_detalle set ver_precio=' . $this->ver_precio. ', usuario_mod_id=' . $this->usuario_mod_id;
            $q.=', fdm=now() where del=0 and id=' . $this->ID;
            $retornar = $cn->transa($q);
            $this->message = 'Se guardó correctamente';
			//echo $q;
    ...
    function eliminar() {
        ...
        try {
            ...
            $q = 'UPDATE orden_venta_detalle SET del=1,usuario_mod_id=' . $this->usuario_mod_id . ', fdm=Now()';
    ...
    static function getByID($ID) {
        $cn = new connect();
        try {
            $q = 'SELECT ID,producto_ID,observacion,orden_venta_ID,descripcion,cantidad,precio_venta_unitario_soles,';
            ...
            $q.='igv,vigv_soles,vigv_dolares,orden_venta_detalle_ID,estado_ID,cotizacion_detalle_ID,ver_precio,obsequio,tipo,usuario_id,ifNull(usuario_mod_id,-1) as usuario_mod_id';
            $q.=' FROM orden_venta_detalle ';
            $q.=' where del=0 and ID='.$ID;
            //echo $q;
            $dt = $cn->getGrid($q);
            $oOrden_Venta_Detalle = null;

            foreach ($dt as $item) {
                $oOrden_Venta_Detalle = new orden_venta_detalle();

                $oOrden_Venta_Detalle->ID=$item['ID'];
                $oOrden_Venta_Detalle->producto_ID=$item['producto_ID'];
                $oOrden_Venta_Detalle->observacion=$item['observacion'];
                $oOrden_Venta_Detalle->orden_venta_ID=$item['orden_venta_ID'];
                $oOrden_Venta_Detalle->descripcion=$item['descripcion'];
                ...
                $oOrden_Venta_Detalle->orden_venta_detalle_ID=$item['orden_venta_detalle_ID'];
                $oOrden_Venta_Detalle->estado_ID=$item['estado_ID'];
                $oOrden_Venta_Detalle->cotizacion_detalle_ID=$item['cotizacion_detalle_ID'];
                $oOrden_Venta_Detalle->ver_precio=$item['ver_precio'];
                $oOrden_Venta_Detalle->obsequio=$item['obsequio'];
                $oOrden_Venta_Detalle->tipo=$item['tipo'];
                $oOrden_Venta_Detalle->usuario_id=$item['usuario_id'];
                $oOrden_Venta_Detalle->usuario_mod_id=$item['usuario_mod_id'];
    ...
    static function getGrid($filtro='',$desde=-1,$hasta=-1,$order='ID asc') {
        ...
        try {
            $q = 'SELECT ID,producto_ID,observacion,orden_venta_ID,descripcion,cantidad,precio_venta_unitario_soles,';
            $q.='precio_venta_unitario_dolares,precio_venta_subtotal_soles,precio_venta_subtotal_dolares,precio_venta_soles,precio_venta_dolares,';
            $q.='igv,vigv_soles,vigv_dolares,orden_venta_detalle_ID,estado_ID,ver_precio,obsequio,cotizacion_detalle_ID,tipo,usuario_id,ifNull(usuario_mod_id,-1) as usuario_mod_id';
            $q.=' FROM orden_venta_detalle ';
            $q.=' where del=0 ';
...
//echo $q;
     ...
     static function getGridLista($filtro='',$desde=-1,$hasta=-1,$order='ovd.ID asc') {
        $cn = new connect();
        try {
            $q = 'select ovd.ID,ovd.orden_venta_ID, ovd.cantidad,ovd.descripcion,pro.nombre as producto, um.nombre as unidad_medida,pro.peso,ovd.ver_precio,';
            ...
            $q.=' from orden_venta_detalle ovd, producto pro, unidad_medida um, orden_venta ov';
            $q.=' where ovd.orden_venta_ID=ov.ID and ovd.producto_ID=pro.ID and pro.unidad_medida_ID=um.ID';
            $q.=' and ovd.del=0 ';
