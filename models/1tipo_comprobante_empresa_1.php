...
class tipo_comprobante_empresa
{
 	...
 	static function getByID($ID)
	{
		...
		try 
		{
			$q='Select ID,tipo_comprobante_ID,empresa_ID,accion,imprime,usuario_id,ifnull(usuario_mod_id,-1) as usuario_mod_id';
	...
	static function getGrid($filtro='',$desde=-1,$hasta=-1,$order='ID asc')
	{
		...
		try 
		{
			$q='SELECT tce.ID,tce.tipo_comprobante_ID,tce.empresa_ID,tce.accion,tce.imprime,tce.usuario_id,ifnull(tce.usuario_mod_id,-1) as usuario_mod_id';
			$q.=',tc.nombre';
		...
		{
                    log_error(__FILE__, "tipo_comprobante_empresa.getGrid", $ex->getMessage().$q);
			throw new Exception('Ocurrio un Error en la consulta');
		}
	}
}
